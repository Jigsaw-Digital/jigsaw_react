...
export default function LayoutHero({block}) {
    let data = block.attrs.data;

    return (
        <Section block={block}>
            <div className={`overflow-hidden pt-20 pb-20 lg:pb-60 ${SectionColours.gray}`}>
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                    <div className="lg:grid lg:grid-cols-12 lg:gap-x-8 lg:gap-y-20">

                        <div className="relative z-10 mx-auto max-w-2xl lg:col-span-7 lg:max-w-none lg:pt-6 xl:col-span-6">
                            <h1 className="text-4xl font-medium tracking-tight"> {data?.title}</h1>
                            <p className="mt-6 text-lg"> {data?.description}</p>

                            <div className="mt-8 flex flex-wrap gap-x-6 gap-y-4">
                                {[...Array(data.links)].map((x, i) => (
                                    <Link href={abs_url(data[`links_${i}_link`].url)}>
                                        <a className="inline-flex justify-center rounded-lg border py-[calc(theme(spacing.2)-1px)] px-[calc(theme(spacing.3)-1px)] text-sm outline-2 outline-offset-2 transition-colors border-gray-300 hover:border-gray-400 active:bg-gray-100 active:text-gray-700/80"
                                        >
                                                <span>
                                                {data[`links_${i}_link`].title}
                                                </span>
                                        </a>
                                    </Link>
                                ))}
                        ...
                        <div className="relative mt-10 sm:mt-20 lg:col-span-5 lg:row-span-2 lg:mt-0 xl:col-span-6">
                            ...
                            <div className="-mx-4 h-[448px] px-9 [mask-image:linear-gradient(to_bottom,white_85%,transparent)] sm:mx-0 lg:absolute lg:-inset-x-10 lg:-top-10 lg:-bottom-20 lg:h-auto lg:px-0 lg:pt-10 xl:-bottom-32">
                        ...
                        <div className="relative -mt-4 lg:col-span-7 lg:mt-0 xl:col-span-6">
                            <p className="text-center text-sm font-semibold lg:text-left">{data.links_with_images_title}</p>
                            <ul role="list" className="mx-auto mt-8 flex max-w-xl flex-wrap justify-center gap-x-10 gap-y-8 lg:mx-0 lg:justify-start">
                                {[...Array(data.links_with_images)].map((x, i) => (
                                    <li key={`hero-image-link-${i}`}
                                        className="col-span-1 flex flex-col divide-y divide-gray-200 rounded-lg bg-white text-center shadow">
                                        <Link href={abs_url(data[`links_with_images_${i}_link`].url)}>
                                            <a>
                                                <img alt={data[`links_with_images_${i}_link`].title}
                                                     src={data[`links_with_images_${i}_image`].url}
                                                     width="82" height="32" decoding="async"
                                                     data-nimg="future" className="h-8"
                                                     loading="lazy"/>
                                            </a>
                                        </Link>
                                    </li>
